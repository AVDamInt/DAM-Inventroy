...
def process_custom(
    custom: Optional[ct.CustomSelectors],
) -> Dict[str, Union[str, ct.SelectorList]]:
    """Process custom."""
...
class CSSParser:
    ...
    def parse_pseudo_class_custom(
        ...
        if selector is None:
            raise SelectorSyntaxError(
                "Undefined custom selector '{}' found at position {}".format(
                    pseudo, m.end(0)
                ),
                self.pattern,
                m.end(0),
    ...
    def parse_combinator(
        ...
        if not has_selector:
            if not is_forgive or combinator != COMMA_COMBINATOR:
                raise SelectorSyntaxError(
                    "The combinator '{}' at position {}, must have a selector before it".format(
                        combinator, index
                    ),
                    self.pattern,
                    index,
        ...
        try:
            while True:
                ...
                if key == "at_rule":
                    ...
                    raise NotImplementedError(
                        "Pseudo-element found at position {}".format(m.start(0))
                    )
                    ...
                    if is_open:
                        ...
                        raise SelectorSyntaxError(
                            "Unmatched pseudo-class close at position {}".format(
                                m.start(0)
                            ),
                            self.pattern,
                            m.start(0),
