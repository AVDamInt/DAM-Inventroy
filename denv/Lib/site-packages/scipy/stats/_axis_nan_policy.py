...
def _axis_nan_policy_factory(
    ...
    def axis_nan_policy_decorator(hypotest_fun_in):
        ...
        def axis_nan_policy_wrapper(*args, _no_deco=False, **kwds):
            ...
            if intersection:
                message = (
                    f"{hypotest_fun_in.__name__}() got multiple values "
            ...
            samples = [
                np.atleast_1d(kwds.pop(param)) for param in (params[:n_samp] + kwd_samp)
            ]
            ...
            if np.all(ndims <= 1):
                ...
                contains_nans = []
                for sample in samples:
                    contains_nan, _ = scipy.stats._stats_py._contains_nan(
                        sample, nan_policy
                    )
            ...
            # Addresses nan_policy == "raise"
            contains_nan, _ = scipy.stats._stats_py._contains_nan(x, nan_policy)
