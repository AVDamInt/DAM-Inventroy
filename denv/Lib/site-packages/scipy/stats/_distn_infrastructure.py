...
class rv_generic:
    ...
    def moment(self, order=None, *args, **kwds):
        ...
        #    error (order of the moment is not provided), and it is still an
        #    error for the same reason.
        if not got_order and (
            (not got_keyword_n) or (got_keyword_n and has_shape_n)  # A4 and B4
        ):  # A2
        ...
        if got_keyword_n and not has_shape_n:
            if got_order:  # B1
                ...
                message = (
                    "Use of keyword argument `n` for method "
    ...
    def interval(self, confidence=None, *args, **kwds):
        ...
        has_shape_alpha = self.shapes is not None and "alpha" in (
            self.shapes.split(", ")
        )
        got_confidence = confidence is not None
        got_keyword_alpha = kwds.get("alpha", None) is not None

        if not got_confidence and (
            (not got_keyword_alpha) or (got_keyword_alpha and has_shape_alpha)
        ):
            message = (
                "interval() missing 1 required positional argument: " "`confidence`"
            )
            raise TypeError(message)
    ...
    **Statistics**
    ...
    def __init__(
        ...
        if extradoc is not None:
            warnings.warn(
                "extradoc is deprecated and will be removed in " "SciPy 1.11.0",
                DeprecationWarning,
            )
