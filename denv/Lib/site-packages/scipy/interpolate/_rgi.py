...
def _make_points_and_values_ascending(points, values):
    ...
    points_asc = tuple(
        np.asarray(point)[sort_index]
        for (point, sort_index) in zip(points, sorted_indexes)
    )

    # create ascending values
    ordered_indexes = tuple([*range(len(x))] for x in sorted_indexes)
    ordered_indexes_array = np.array(
        [i.flatten() for i in np.meshgrid(*ordered_indexes)]
    ).transpose()
    sorted_indexes_array = np.array(
        [i.flatten() for i in np.meshgrid(*sorted_indexes)]
    ).transpose()
    ...
    **Interpolate and extrapolate a 2D dataset**
    ...
    def __init__(
        ...
        for i, p in enumerate(points):
            diff_p = np.diff(p)
            if not np.all(diff_p > 0.0):
                if np.all(diff_p < 0.0):
    ...
    def __call__(self, xi, method=None):
        ...
        if method == "linear":
            ...
            result = self._evaluate_spline(
                self.values.T, xi, self._SPLINE_DEGREE_MAP[method]
            )
    ...
    def _evaluate_spline(self, values, xi, spline_degree):
        ...
        # last variable of xi.
        last_dim = n - 1
        first_values = self._do_spline_fit(
            self.grid[last_dim], values, xi[:, last_dim], spline_degree
        )
        ...
        for j in range(m):
            ...
            for i in range(last_dim - 1, -1, -1):
                ...
                folded_values = self._do_spline_fit(
                    self.grid[i], folded_values, xi[j, i], spline_degree
                )
